...
dev2 = torch.device('cuda:7' if torch.cuda.is_available() else 'cpu')
...
gnn_dataset_train_pos= create_me_a_gnn_dataset_you_stupid([train_pos_node_cover], [train_pos_labels], group_size=100, verses=shuffled_verses[:])
...
m_path = 'POSTAG_15lngs-POSFeatFalsealltgts_trnsfrmrFalse6LResTrue_trainWEFalse_mskLngTrue_E1_traintgt0.8_TypchckTrue_TgBsdSlctTrue_tstamtall_20220511-202211_ElyStpDlta0-GA-chnls1024_small_final_.pickle'
...
#res_ = torch.load(f'/mounts/work/ayyoob/results/gnn_postag/data/15lngs-POSFeatFalsealltgts_trnsfrmrFalse6LResTrue_trainWEFalse_mskLngTrue_E1_traintgt0.8_TypchckTrue_TgBsdSlctTrue_tstamtall_20220511-202211_ElyStpDlta0-GA-chnls1024_small_final_th0.98_typchkFalse.torch.bin')
...
#for lang in pos_test_lang_list[:10]:
train_langs = pos_val_lang_list[:]
train_langs.extend(pos_lang_list)
train_langs.append(lang)
params = f'traintgt{threshold}_Typchck{type_check}_WITHXLMR'
#filtered_nodes_cover = filter_node_cover_for_languages(train_pos_node_covers_ext, train_dataset.nodes_map, train_langs)

#gnn_dataset_train_pos_ext = create_me_a_gnn_dataset_you_stupid([train_pos_node_covers_ext], [train_pos_labels_ext], group_size=100)
...
#        tag_frequencies=True, params=params, residual_connection=True)

m_path = 'POSTAG_15lngs-POSFeatTruealltgts_trnsfrmrTrue6LResTrue_trainWEFalse_mskLngTrue_E1_traintgt0.98_TypchckFalse_WITHXLMR_20220527-110612_ElyStpDlta0-GA-chnls1024_small_final_.pickle'
model = torch.load(f'/mounts/work/ayyoob/models/gnn/checkpoint/postagging/{m_path}', map_location=torch.device('cpu'))
model.to(dev)
model.decoder.to(dev2)
