...
#w2v_model = Word2Vec.load("/mounts/work/ayyoob/models/w2v/word2vec_POS1_langs_10e.model")
...
# pos_lang_list = ["eng-x-bible-mixed", "deu-x-bible-bolsinger", "spa-x-bible-hablahoi-latina", "fra-x-bible-louissegond", "rus-x-bible-newworld", "arb-x-bible"]
...
gnn_dataset_train_pos= create_me_a_gnn_dataset_you_stupid([train_pos_node_cover], [train_pos_labels], group_size=64, verses=shuffled_verses[:])

gnn_dataset_train_pos_bigbatch = create_me_a_gnn_dataset_you_stupid([train_pos_node_cover], [train_pos_labels], group_size=10000, verses=shuffled_verses[:])
...
m_path = 'POSTAG_15lngs-POSFeatFalsealltgts_trnsfrmrFalse6LResTrue_trainWEFalse_mskLngTrue_E1_traintgt0.8_TypchckTrue_TgBsdSlctTrue_tstamtall_NOXLMR_20220511-205450_ElyStpDlta0-GA-chnls1024_small_final_.pickle'
...
#res_ = torch.load(f'/mounts/work/ayyoob/results/gnn_postag/data/15lngs-POSFeatFalsealltgts_trnsfrmrFalse6LResTrue_trainWEFalse_mskLngTrue_E1_traintgt0.8_TypchckTrue_TgBsdSlctTrue_tstamtall_NOXLMR_20220511-205450_ElyStpDlta0-GA-chnls1024_small_final_th0.95_typchkFalse.torch.bin')
...
#                                [target_data_loader_train], [train_data_loader_bigbatch], DataEncoder, source_tag_frequencies=tag_frequencies_source,
#                                target_train_treshold=threshold, type_check=type_check)

#print('saving at', f'/mounts/work/ayyoob/results/gnn_postag/data/{model.model_name}_th{threshold}_typchk{type_check}.torch.bin')
...
# %%
# Final Bronze3, with xlmr, all training langs, testlangset1
importlib.reload(posutil)
XLMR = False
POSTAG = True
WORDEMBEDDING = True
...
params = f'traintgt{threshold}_Typchck{type_check}_small_NOXLMR'
#filtered_nodes_cover = filter_node_cover_for_languages(train_pos_node_covers_ext, train_dataset.nodes_map, train_langs)

#gnn_dataset_train_pos_ext = create_me_a_gnn_dataset_you_stupid([train_pos_node_covers_ext], [train_pos_labels_ext], group_size=16)
...
#        tag_frequencies=True, params=params, residual_connection=False)


m_path = 'POSTAG_15lngs-POSFeatTruealltgts_trnsfrmrTrue6LResFalse_trainWEFalse_mskLngTrue_E1_traintgt0.95_TypchckFalse_small_NOXLMR_20220527-110844_ElyStpDlta0-GA-chnls1024_small_final_.pickle'
model = torch.load(f'/mounts/work/ayyoob/models/gnn/checkpoint/postagging/{m_path}', map_location=torch.device('cpu'))
model.to(dev)
model.decoder.to(dev2)
