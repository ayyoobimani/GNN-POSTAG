"""
POS tagger with Character and Fasttext embeddings

$ python3 pos_tagger.py --bronze 1 --lang tam --fasttext cc.ta.300.vec --gpu 6
$ python3 pos_tagger.py --bronze 1 --lang fin --fasttext cc.fi.300.vec --gpu 6
...
def train(bronze, lang, fasttext):
    ...
    if not os.path.isfile(fasttext+'.gensim'):
        print("Loading embeddings...")
        word_vectors = gensim.models.KeyedVectors.load_word2vec_format(fasttext, binary=False)
        word_vectors.save(fasttext+'.gensim')
    print("Embeddings loaded")
    fasttext_embeddings = WordEmbeddings(fasttext+'.gensim')
    ...
    # 7. start training
    trainer.train('resources/taggers/'+lang+'-upos-char-fasttext-'+bronze,
                learning_rate=0.1,
